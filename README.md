...
3. Set `VITE_CONTRACT_ID` in your frontend `.env` file (if using direct contract calls)
4. Ensure `SERVER_PRIVATE_KEY` is set in your backend `.env` file
5. Regenerate contract bindings: `CONTRACT_ID=$(cat contracts/contract_id.txt) npm run gen:bindings`
6. Commit the updated bindings to the repository
...
- `frontend/src/contracts/generated/`: TypeScript bindings generated by npm run gen:bindings (committed to repository).
